...
\end{align}
What \eq{eqT:hc} and \eq{eqT:hp} tells is how the waveform of the emitted GW signal depends on from witch direction we are viewing the source.
...
in the coordinate system of the simulations, and then use \eq{eqT:hc} and \eq{eqT:hp} to compute the signal an observer situated along the
vector $\hat{n'} = (\sin{\theta} \sin{\phi}, \sin{\theta} \cos{\phi},\cos{\theta})$ would observe.

In principle we have all the ingredients we need to calculate the gravitational quadrupole radiation for a slow-moving source.
...
quadrupole moment from numerical simulations. Furthermore, in \eq{eqT:quad} the terms $\rho x_i x_j \mathrm{d} V$ will give large weight
...
$\partial_k x_i = \delta^{ki}$. We have removed the time derivative from \eq{eqT:quad} and we have replaced one
power of $x^i$ with $v^i$ which reduces the weight given to slow moving regions.
Next we take the time derivative of \eq{eqT:quaddt2}
